...
export const login = async (req, res) => { 
    try {
        ...
        console.log(new Date(), process.pid, `${email} login.`);

        const existUser = await UserModal.findOne({ email });

        if (!existUser) {
          console.log(new Date(), process.pid, `${email} user dones't exist.`);
          return res.status(404).json(processResponseData(404, [], USER_NOT_EXIST, `User dones't exist.`));
        }

        console.log(new Date(), process.pid, `${existUser.email} is exist, check password.`);

        const isPasswordMatched = hash(password) === existUser.password;

        if (!isPasswordMatched) {
          console.log(new Date(), process.pid, `${existUser.email} passwrod check failed.`);
        ...
        console.log(new Date(), process.pid, `${existUser.email} login success`);

				console.log(new Date(), process.pid, `Set user token to redis, key: uid-${existUser._id}`);
        ...
        console.error(new Date(), process.pid, 'Login failed with error: ', error.message);
...
export const register = async (req, res) => {
    ...
    try {
        console.log(new Date(), process.pid, `${email} register.`);

        const existUser = await UserModal.findOne({ email });

        if (existUser) {
          console.log(new Date(), process.pid, `${email} user already exist.`);
        ...
        console.log(new Date(), process.pid, `Creating ${email} user.`);
        ...
        console.log(new Date(), process.pid, `Create ${email} user success.`);
        ...
        console.error(new Date(), process.pid, 'Register failed with error: ', error.message);
        res.status(500).json(resData);
    }
}
